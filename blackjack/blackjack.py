...
def deal_card(frame):
    ...
    # add the image to Label and gisplay the label
...
def deal_player():
    global stay_flag
    player_score = score_hand(player_hand)
    if player_score < 21 and not stay_flag:
...
def newgame():
    ...
    # enbended new pack cards
    deck = list(cards)
    random.shuffle(deck)
    # deal the new game cards
    player_hand.append(deal_card(player_card_frame))
...
# embended frame to hold the card images
player_card_frame = tkinter.Frame(card_frame, background="green")
player_card_frame.grid(row=2, column=1, sticky="ew", rowspan=2)
...
surrender_button = tkinter.Button(button_frame, text="Stay", command=deal_dealer)
surrender_button.grid(row=0, column=1)
...
# create the list to store the dealer`s anf player`s hands
dealer_hand = []
player_hand = []
